...
raw_court_features = {
    'lc_direction': 'lcDispositionDirection',
...
def get_ml_row(docket_id, scdb_case_data, scdb_justice_data):
    ...
    court_direction_data = get_means(
    ...
    court_issue_data = get_data_by_condition(
        docket_issue,
        'issue',
        docket_before_case_data
    )

    court_direction_issue_data = get_means(
        court_issue_data,
        'decisionDirection',
        'court_direction_issue',
        windows=ROLL_WINDOWS
    )

    court_circuit_origin_data = get_data_by_condition(
        docket_circuit_origin,
        'caseOrigin_circuit',
        docket_before_case_data
    )

    court_direction_circuit_origin_data = get_means(
        court_circuit_origin_data,
        'decisionDirection',
        'court_direction_circuit_origin',
        windows=ROLL_WINDOWS
    )

    court_circuit_source_data = get_data_by_condition(
        docket_circuit_source,
        'caseSource_circuit',
        docket_before_case_data
    )

    court_direction_circuit_source_data = get_means(
        court_circuit_source_data,
        'decisionDirection',
        'court_direction_circuit_source',
        windows=ROLL_WINDOWS
    )

    court_petitioner_data = get_data_by_condition(
        docket_petitioner,
        'petitioner',
        docket_before_case_data
    )

    court_direction_petitioner_data = get_means(
        court_petitioner_data,
        'decisionDirection',
        'court_direction_petitioner',
        windows=ROLL_WINDOWS
    )

    court_respondent_data = get_data_by_condition(
        docket_respondent,
        'respondent',
        docket_before_case_data
    )

    court_direction_respondent_data = get_means(
        court_respondent_data,
        'decisionDirection',
        'court_direction_respondent',
        windows=ROLL_WINDOWS
    )
    ...
    lower_court_issue_data = get_data_by_condition(
        docket_issue,
        'issue',
        docket_before_case_data
    )

    lower_court_direction_issue_data = get_means(
        court_issue_data,
        'lcDispositionDirection',
        'lower_court_direction_issue',
        windows=ROLL_WINDOWS
    )

    lower_court_circuit_source_data = get_data_by_condition(
        docket_circuit_source,
        'caseSource_circuit',
        docket_before_case_data
    )

    lower_court_direction_circuit_source_data = get_means(
        court_circuit_source_data,
        'lcDispositionDirection',
        'lower_court_direction_circuit_source',
        windows=ROLL_WINDOWS
    )

    lower_court_petitioner_data = get_data_by_condition(
        docket_petitioner,
        'petitioner',
        docket_before_case_data
    )

    lower_court_direction_petitioner_data = get_means(
        court_petitioner_data,
        'lcDispositionDirection',
        'lower_court_direction_petitioner',
        windows=ROLL_WINDOWS
    )

    lower_court_respondent_data = get_data_by_condition(
        docket_respondent,
        'respondent',
        docket_before_case_data
    )

    lower_court_direction_respondent_data = get_means(
        court_respondent_data,
        'lcDispositionDirection',
        'lower_court_direction_respondent',
        windows=ROLL_WINDOWS
    )
    ...
    for justice_group_name, justice_group_data in docket_before_justice_data.groupby('justice'):
        ...
        justice_direction_data = get_means(
            justice_group_data,
            'direction',
            'justice_direction',
            windows=ROLL_WINDOWS
        ...
        justice_group_issue_data = get_data_by_condition(
            docket_issue,
            'issue',
            justice_group_data
        )

        justice_direction_issue_data = get_means(
            justice_group_issue_data,
            'direction',
            'justice_direction_issue',
            windows=ROLL_WINDOWS
        )

        justice_group_circuit_origin_data = get_data_by_condition(
            docket_circuit_origin,
            'caseOrigin_circuit',
            justice_group_data
        )

        justice_direction_circuit_origin_data = get_means(
            justice_group_circuit_origin_data,
            'direction',
            'justice_direction_circuit_origin',
            windows=ROLL_WINDOWS
        )

        justice_group_circuit_source_data = get_data_by_condition(
            docket_circuit_source,
            'caseSource_circuit',
            justice_group_data
        )

        justice_direction_circuit_source_data = get_means(
            justice_group_circuit_source_data,
            'direction',
            'justice_direction_circuit_source',
            windows=ROLL_WINDOWS
        )

        justice_group_petitioner_data = get_data_by_condition(
            docket_petitioner,
            'petitioner',
            justice_group_data
        )

        justice_direction_petitioner_data = get_means(
            justice_group_petitioner_data,
            'direction',
            'justice_direction_petitioner',
            windows=ROLL_WINDOWS
        )

        justice_group_respondent_data = get_data_by_condition(
            docket_respondent,
            'respondent',
            justice_group_data
        )

        justice_direction_respondent_data = get_means(
            justice_group_respondent_data,
            'direction',
            'justice_direction_respondent',
            windows=ROLL_WINDOWS
        )
